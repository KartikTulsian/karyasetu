...
import { Result, User, Event, GroupType } from "@prisma/client";
        ...
        <div className="p-6 space-y-6">
          ...
          {/* <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg shadow-sm">
              <Image src="/result.png" alt="" width={25} height={25}/>
              <span><strong>result Type:</strong> {result.result_type}</span>
            </div>
            <div className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg shadow-sm">
              <Image src="/group.png" alt="" width={25} height={25}/>
              <span><strong>Target Group:</strong> {result.target_group_type}</span>
            </div>
            <div className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg shadow-sm">
              <Image src="/college.png" alt="" width={25} height={25}/>
              <span><strong>College:</strong> {result.target_college_name || "N/A"}</span>
            </div>
            <div className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg shadow-sm">
              <Image src="/date.png" alt="" width={25} height={25}/>
              <span><strong>Date Created:</strong> {new Intl.DateTimeFormat("en-US").format(new Date(result.created_at))}</span>
            </div>
          </div> */}
          ...
          {result.event && (
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 shadow-sm">
              <h3 className="font-semibold text-purple-800 mb-2">🎉 Result of Event</h3>
              <p><strong>Title:</strong> {result.event.title}</p>
